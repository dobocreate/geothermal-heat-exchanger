# 地中熱交換簡易シミュレーター - Claude開発ガイド

このドキュメントは、Claudeがこのプロジェクトを理解し、効果的に開発支援を行うためのガイドです。

## プロジェクト概要

地上の温水を地盤の安定した温度環境で冷却する環境配慮型システムの設計支援ツールです。Streamlitを使用したWebアプリケーションで、主に冷房運転時のエアコン排熱処理を想定し、地下水との熱交換による温度低下を計算します。U字管構造を前提とした配管セット単位での計算を行います。

**ターゲットユーザー**: 土木エンジニア（地中熱交換の専門知識は不要）
**主な用途**: 冷却塔の代替として省スペース・省エネ・低騒音を実現するシステムの設計検討
...
### 土木エンジニア向け大幅改善（2025-01-08）
- **タイトル変更**: 「地中熱交換簡易シミュレーター」に変更
- **説明文改善**: 環境配慮型システムの価値を強調
- **専門用語説明**: 全ての詳細パラメータにhelpテキストを追加
- **理論解説大幅改訂**: 土木エンジニア向けに全面的に書き直し
  - システム概念の詳細説明
  - U字管構造の利点を施工・維持管理・効率の観点で説明
  - 実務的な設計指針と目標値を明記
  - よくある設計質問をQ&A形式で追加
  - 専門用語を分かりやすい表現に変更
- **UI改善**: 重要指標（出口温度、地下水温、通水時間）を枠線で強調
- **スライダー同期**: 数値入力とスライダーの完全同期機能

### 過去の更新（v1.3.0 - 2025-01-06）
- セクション間に視覚的区切り線を追加
